...
export class ArticlesListScreen extends ListScreen {
  ...
  fetchData() {
    const { feedUrl, shortcut, categoryName } = this.props;
    const { perPage } = this.state;

    if (_.isEmpty(feedUrl) || _.isEmpty(categoryName)) {
      return;
    }
...
export const mapStateToProps = (state, ownProps) => {
  ...
  const categoryName = _.get(ownProps, 'shortcut.settings.categoryName');
  const { shortcut } = ownProps;

  return {
    feedUrl,
    shortcut,
    data: getFeedItems(state, feedUrl, categoryName),
    categoryName,
  };
};
